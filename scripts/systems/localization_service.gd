...
const STRINGS := {
	...
	"run.summary.route": {"en": "Route: %s", "zh_CN": "路线：%s"},
	"run.summary.reward_line": {"en": "Gold +%d   Souls +%d   Talent Points +%d", "zh_CN": "金币 +%d   灵魂 +%d   天赋点 +%d"},
	"route.title": {"en": "Choose Next Room", "zh_CN": "选择下一房间"},
	"route.subtitle": {"en": "Floor %d route", "zh_CN": "第 %d 层路线"},
	"route.choice": {"en": "Floor %d: %s\n%s - %s", "zh_CN": "第 %d 层：%s\n%s - %s"},
	"route.unknown": {"en": "Unknown Route", "zh_CN": "未知路线"},
	"route.hint.combat": {"en": "Fight a balanced pack for a reward chest.", "zh_CN": "击败常规敌群，赢取奖励宝箱。"},
	"route.hint.elite": {"en": "Face an elite-centered room for stronger rewards.", "zh_CN": "挑战精英核心房间，换取更强奖励。"},
	"route.hint.treasure": {"en": "Claim a cache without a full fight.", "zh_CN": "领取宝箱奖励，不进行完整战斗。"},
	"route.hint.forge": {"en": "Use the forge to push an item further.", "zh_CN": "使用锻造台，继续强化装备。"},
	"route.hint.event": {"en": "Take a risk-reward shrine choice.", "zh_CN": "进行一次风险与收益并存的事件选择。"},
	"route.hint.boss": {"en": "Challenge a major guardian.", "zh_CN": "挑战关键守卫。"},
	"route.hint.mini_boss_room": {"en": "Break the floor-five armor gate.", "zh_CN": "突破第 5 层护甲关口。"},
	"route.hint.boss_room": {"en": "Finish the ten-floor slice.", "zh_CN": "完成 10 层垂直切片。"},
